...
rpic = Plugin.on_msg(parser=PARSER_GEN.gen(target=['随机图', 'rpic']), checker=COMMON_CHECKER)
...
class RandomPicGen(Plugin):
    ...
    async def random_pic(self) -> None:
        ...
        async with self.get_pic_lock:
            async with async_http(url=choice(self.url_gens)(), method='get', headers=get_headers()) as resp:
                if resp.status != 200:
