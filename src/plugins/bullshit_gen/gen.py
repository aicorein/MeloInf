...
path = str(Path(__file__).parent.joinpath("data.json"))
...
class BullShitGenerator:
    ...
    def __init__(self, theme: str, words_len: int) -> None:
        self.名人名言 = BullShitGenerator.DATA[
            "famous"
        ].copy()  # a 代表前面垫话，b代表后面垫话
        self.前面垫话 = BullShitGenerator.DATA["before"].copy()  # 在名人名言前面弄点废话
        self.后面垫话 = BullShitGenerator.DATA["after"].copy()  # 在名人名言后面弄点废话
        self.废话 = BullShitGenerator.DATA["bosh"].copy()  # 代表文章主要废话来源
        self.xx = theme
        self.theme = theme
        self.重复度 = 2
